...
function gamehub_security_client_ip(): string
{
...
function gamehub_security_client_data(): array
{
...
function gamehub_security_sanitize_scalar($value, int $max = 255): ?string
{
    if ($value === null || is_array($value) || is_object($value)) return null;
    return mb_substr(trim((string)$value), 0, $max, 'UTF-8');
}

function gamehub_security_ensure_schema(PDO $pdo): void
{
    static $done = false;
    if ($done) return;
    try {
...
function gamehub_security_identifier(string $identifier): string
{
    return mb_substr(mb_strtolower(trim($identifier), 'UTF-8'), 0, 120, 'UTF-8');
}

function gamehub_security_record_event(PDO $pdo, string $type, string $identifier = '', ?int $userId = null): void
{
    try {
        gamehub_security_ensure_schema($pdo);
        $data = gamehub_security_client_data();
        ...
        $stmt->execute([
            ...
            ':identifier' => gamehub_security_identifier($identifier),
            ':ip' => gamehub_security_client_ip(),
...
function gamehub_security_is_rate_limited(PDO $pdo, string $identifier = ''): bool
{
    try {
        gamehub_security_ensure_schema($pdo);
        $ip = gamehub_security_client_ip();
        $id = gamehub_security_identifier($identifier);
...
function gamehub_turnstile_enabled(): bool
{
    return trim((string)getenv('TURNSTILE_SITE_KEY')) !== '' && trim((string)getenv('TURNSTILE_SECRET_KEY')) !== '';
}

function gamehub_turnstile_site_key(): string
{
    return trim((string)getenv('TURNSTILE_SITE_KEY'));
}

function gamehub_turnstile_verify(): bool
{
    if (!gamehub_turnstile_enabled()) return true;
    ...
    $payload = http_build_query([
        ...
        'response' => $token,
        'remoteip' => gamehub_security_client_ip(),
    ]);
...
function gamehub_security_telemetry_script(string $formSelector = 'form'): string
{
