...
async function startServerSession() {
  try {
    const r = await fetch("/api/flappy/start.php", {
      method: "POST"
    });
    ...
    if (!r.ok || !data.ok) {
      console.error("Erro ao iniciar partida:", data);
      gameSession = null;
      return;
    }
...
function registerCheckpoint() {
  ...
  checkpointQueue = checkpointQueue.then(async () => {
    const r = await fetch("/api/flappy/checkpoint.php", {
      method: "POST",
      headers: {
    ...
    if (!r.ok || !data.ok) {
      console.error("Checkpoint rejeitado:", data);
      return;
    }
...
async function finishServerSession() {
  ...
  try {
    ...
    const r = await fetch("/api/flappy/finish.php", {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
    ...
    if (!r.ok || !data.ok) {
      console.error("Erro ao finalizar partida:", data);
      return;
    }
    ...
    console.log("Partida salva:", data);
  } catch (e) {
...
class Bird {
  ...
  updateAnimation() {
    ...
    if (this.frameCount < this.animationTime) {
      this.frame = 0;
    } else if (this.frameCount < this.animationTime * 2) {
      this.frame = 1;
    } else if (this.frameCount < this.animationTime * 3) {
      this.frame = 2;
    } else if (this.frameCount < this.animationTime * 4) {
      this.frame = 1;
    } else {
      this.frame = 0;
      this.frameCount = 0;
    ...
    if (this.angle >= 70) {
      this.frame = 1;
      this.frameCount = this.animationTime * 2;
    }
  }
...
class Floor {
  ...
  move() {
    ...
    if (this.x1 + FLOOR_TILE_W < 0) {
      this.x1 = this.x2 + FLOOR_TILE_W;
    }

    if (this.x2 + FLOOR_TILE_W < 0) {
      this.x2 = this.x1 + FLOOR_TILE_W;
    }
  }
