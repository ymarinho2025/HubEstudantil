...
class Grid {
    constructor(rows = 4, cols = 8) {
        this.rows = Math.min(5, Math.max(3, rows));
        this.cols = Math.min(9, Math.max(6, cols));
        this.margin = 96;
    ...
    reachedRightBoundary() {
        return this.invaders.some(
            (invader) => invader.position.x + invader.width >= innerWidth - this.margin
        );
    }

    reachedLeftBoundary() {
        return this.invaders.some((invader) => invader.position.x <= this.margin);
    }
