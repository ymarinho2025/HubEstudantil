...
const showGameOverScreen = () => {
    window.parent.postMessage({type:"gamehub:gameover", score:gameData.score}, window.location.origin);
...
const gameLoop = () => {
    if (gamehubPaused) { requestAnimationFrame(gameLoop); return; }
...
canvas.addEventListener("pointerdown", () => {
    if (currentState === GameState.PLAYING && player.alive) player.shoot(playerProjectiles);
});
...
window.addEventListener("message", (event) => {
  ...
  if (event.data.type === "gamehub:continue") { gamehubPaused=false; if(currentState===GameState.GAME_OVER) restartGame(); }
  if (event.data.type === "gamehub:restart") { gamehubPaused=false; restartGame(); }
});